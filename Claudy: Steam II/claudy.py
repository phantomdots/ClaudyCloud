...
class OverlayFrame(QFrame):
    ...
    def mousePressEvent(self,e):
        for name in ("m_input","m_desc","m_type_custom","search_input"):
...
class Window(QMainWindow):
    ...
    def _build(self):
        ...
        self.refresh_btn=AddButton(self.main_panel,"Обновить",self._refresh_count,
        ...
        make_card("Проверить обновления",
                  ...
                  self.check_btn)
        self.auto_switch=Switch(self._auto_toggled,initial=True)
        ...
        self.modal=box(self.overlay,320,120,640,480); self.overlay.modal=self.modal
        ...
        flbl(self.modal,24,282,"Описание",13)
        self.m_desc=qfield(self.modal,24,308,"Введите описание",84,width=450)
        self.m_desc.textEdited.connect(self._on_change)
        flbl(self.modal,24,348,"Тип запуска",15)
        self._chips=[]
        for i,txt in enumerate([".exe",".msi",".bat",".torrent"]+["Другой"]):
            ...
            geo(chip,496 if txt=="Другой" else 24+i*102,376,w,30)
            self._chips.append(chip)
        self.m_type_custom=qfield(self.modal,496,342,"Пример: .txt",17,width=120)
        ...
        self.add_btn.setFixedSize(px(450),px(30)); geo(self.add_btn,24,416,450,30)
        m_cancel=ChoiceButton("Отмена",self._close_modal,w=120)
        m_cancel.setParent(self.modal); m_cancel.setFixedSize(px(120),px(30))
        geo(m_cancel,496,416,120,30); self.m_cancel=m_cancel
        self.m_error=lbl("",self.modal); geo(self.m_error,24,450,592,24)
    ...
    def _reset_settings(self):
        self.auto_switch.set_state(True); self.dev_switch.set_state(False)
    ...
    def _layout_header(self):
        ...
        self.search_input.show(); self.refresh_btn.show()
        if self.dev_on:
            geo(self.search_input,576,14,150,30)
            geo(self.refresh_btn,732,14,140,30)
            self.new_project_btn.show(); geo(self.new_project_btn,880,14,140,30)
        else:
            geo(self.search_input,672,14,200,30)
            geo(self.refresh_btn,880,14,140,30)
    ...
    def _custom_ok(self):
        ...
        return (len(s)>=2 and s[0]=="."
                and all("a"<=ch.lower()<="z" for ch in s[1:]))
        ...
        def work():
            try:
                ...
                if not owner:
                    ...
                    data={"message":f"Добавить {name}/{fname}",
                          "content":base64.b64encode(raw).decode("utf-8"),
                          "branch":"main"}
                    path=quote(f"{name}/{fname}",safe="/")
                    s,d=_gh("PUT",GIT+f"repos/{owner}/ClaudyCloud/contents/{path}",
                            data,timeout=60)
                    if s in (200,201): msg="Проект добавлен."
                    else: msg="Ошибка: "+str(d.get("message","Неизвестная ошибка"))
    ...
    def _finish_add(self,msg):
        ...
        if not msg.startswith("Ошибка"):
            self.add_btn.set_enabled(True)
            self._reset_modal()
    def _auto_toggled(self,on): return
        ...
        def work():
            try:
                ...
                if not owner: n=-1
                    ...
                    if s==200:
                        n=sum(1 for e in d.get("tree",[])
                              if e.get("type")=="blob")
                    elif s==409: n=0
                    else: n=-1
            except Exception: n=-1
            QTimer.singleShot(0,self,lambda: self._show_count(n))
        threading.Thread(target=work,daemon=True).start()

    def _show_count(self,n):
        self._busy=False
        self.refresh_btn.setText(f"Файлов: {n}" if n>=0 else "Ошибка")
        QTimer.singleShot(3500,lambda: self.refresh_btn.setText("Обновить"))

    # ---- карточки/каталог (пустой) ----

    def _update_catalog_view(self):
        self.catalog_empty.setVisible(self.current_tab=="Каталог")
    ...
    def _reset_modal(self):
        self.selected_file=None; self._type=None
        self.m_input.clear(); self.m_desc.clear(); self.m_type_custom.clear()
    ...
    def _nav_clicked(self,item):
        ...
        tab=item.tt
        self.catalog_empty.setVisible(tab=="Каталог")
